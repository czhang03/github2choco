        ...
        if ($Force) 
        {
            foreach ($packageName in $packageNames) 
            {
                ...
                switch ($profile.$packageName.packageType) {
                    'installer' { Update-InstallerChocoPackage -packageName $packageName -Force}
                    'vsix' {Update-VsixChocoPackage -packageName $packageName -Force}
                    'webFile' {Update-WebFileChocoPackage -packageName $packageName -Force}
                    'zip' {Update-ZipChocoPackage -packageName $packageName -Force}
